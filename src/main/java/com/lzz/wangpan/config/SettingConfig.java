...
import com.lzz.wangpan.util.CommonUtils;
...
public class SettingConfig {
    ...
    public static int[] getAuth(String jsonPath) {
        ...
        for (int i = 0; i < ConfigConsts.AUTH_OF_SETTINGS.length; i++) {
            ...
            auth[i] = WangpanApplication.settings.getBooleanUseEval(key) ? 1 : 0;
        }
        return auth;
    }
    ...
    public static String getUploadStoragePath() {
        ...
        String formatWay = WangpanApplication.settings.getStringUseEval(ConfigConsts.UPLOAD_FORM_OF_SETTING);
    ...
    public static String getStoragePath(String path) {
        ...
        return CommonUtils.checkPath(WangpanApplication.settings.getStringUseEval(path));
    }
