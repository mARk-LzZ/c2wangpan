...
public class FileController {
    ...
    public String getUserUploaded(int offset, String search) {
        User user = (User) request.getSession().getAttribute(ValueConsts.USER_STRING);
        System.out.println(Formatter.listToJson(fileService.listUserUploaded(user.getId(), offset, search)));
        return String.valueOf(fileService.listUserUploaded(user.getId(), offset, search));
    }
    ...
    public String getAll(int offset, int categoryId, String orderBy, String search) {
        User user = (User) request.getSession().getAttribute(ValueConsts.USER_STRING);
        boolean canGet = WangpanApplication.settings.getBooleanUseEval(ConfigConsts.ANONYMOUS_VISIBLE_OF_SETTING) ||
