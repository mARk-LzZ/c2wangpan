...
public class ConfigController {
    ...
    public String updateConfig(String config) {
        ...
        if (user.getPermission() > ValueConsts.TWO_INT) {
            WangpanApplication.settings.setJsonObject(config);
            //打包成jar之后无法修改config.json文件
            try {
                FileExecutor.saveFile(NetUtils.urlToString(WangpanApplication.class.getResource(DefaultValues
                        .SETTING_PATH)), WangpanApplication.settings.toString());
            } catch (IOException e) {
    ...
    public String getAll() {
        User user = (User) request.getSession().getAttribute(ValueConsts.USER_STRING);
        if (user.getPermission() > ValueConsts.TWO_INT) {
            return WangpanApplication.settings.toString();
        } else {
            return "{\"message\":\"permission denied\"}";
        }
    }
