...
import com.lzz.wangpan.mapper.FileMapper;
import com.lzz.wangpan.entity.Category;
import com.lzz.wangpan.entity.File;
import com.lzz.wangpan.entity.User;
import com.lzz.wangpan.model.AuthRecord;
import com.lzz.wangpan.model.BaseAuthRecord;
import com.lzz.wangpan.model.FileBasicRecord;
import com.lzz.wangpan.model.FileRecord;
...
import com.lzz.wangpan.service.IFileService;
...
public class FileServiceImpl implements IFileService {
    ...
    @Autowired
    public FileServiceImpl(FileMapper fileMapper, ICategoryService categoryService, IAuthService authService,
                           DownloadedMapper downloadDAO) {
    ...
    public boolean updateAuth(long id, String auth) {
        ...
        return fileMapper.updateAuthById(id, au[0], au[1], au[2], au[3], au[4]);
    }

    @Override
    public BaseAuthRecord getAuth(long id) {
        return fileMapper.getAuth(id);
    }
    ...
    public boolean deleteFiles(String ids) {
        if (Checker.isNotEmpty(ids)) {
            ...
            for (String s : id) {
                ...
                String localUrl = fileMapper.getLocalUrlById(fileId);
    ...
    public boolean[] updateUrl(int id, String oldLocalUrl, String localUrl, String visitUrl) {
        ...
        if (canUpdateLocalUrl) {
            ...
            fileMapper.updateLocalUrlById(id, localUrl);
            b[0] = true;
        }
        if (Checker.isNotEmpty(visitUrl) && !visitUrlExists(visitUrl)) {
            fileMapper.updateVisitUrlById(id, visitUrl);
            b[1] = true;
        }
        return b;
    }
    ...
    public boolean updateFileInfo(long id, User user, String name, String category, String tag, String description) {
        ...
        if (Checker.isNotNull(file) && file.getIsUpdatable() == 1) {
            ...
            boolean canUpdate = (Checker.isNull(authRecord) ? user.getIsUpdatable() == 1 :
                    authRecord.getIsUpdatable() == 1) && Checker.isNotEmpty(name) && Pattern.compile(WangpanApplication.settings.getStringUseEval(ConfigConsts.FILE_SUFFIX_MATCH_OF_SETTING)).matcher(suffix).matches();
            if (canUpdate) {
                ...
                if (isValid && fileMapper.updateFileInfo(file)) {
    ...
    public List<FileRecord> listUserDownloaded(int userId, int offset, String search) {
        return fileMapper.listUserDownloaded(userId, offset, search);
    }

    @Override
    public List<FileRecord> listUserUploaded(int userId, int offset, String search) {
        return fileMapper.listUserUploaded(userId, offset, search);
    }
    ...
    public boolean removeByVisitUrl(String visitUrl) {
        long fileId = fileMapper.getIdByVisitUrl(visitUrl);
        return removeById(fileId);
    }

    @Override
    public boolean removeByLocalUrl(String localUrl) {
        long fileId = fileMapper.getIdByLocalUrl(localUrl);
        return removeById(fileId);
    }
    ...
    public String getResource(String visitUrl, HttpServletRequest request) {
        ...
        boolean downloadable = WangpanApplication.settings.getBooleanUseEval(ConfigConsts
                .ANONYMOUS_DOWNLOADABLE_OF_SETTING);
        User user = (User) request.getSession().getAttribute(ValueConsts.USER_STRING);
        File file = fileMapper.getFileByVisitUrl(visitUrl);
        ...
        boolean canDownload = Checker.isNotNull(file) && file.getIsDownloadable() == 1 && (downloadable || (Checker
                ...
                .getIsDownloadable() == 1));
        if (canDownload) {
            fileMapper.updateDownloadTimesById(file.getId());
    ...
    public String getLocalUrlByVisitUrl(String visitUrl) {
        return fileMapper.getLocalUrlByVisitUrl(Checker.checkNull(visitUrl));
    }

    @Override
    public List<FileRecord> listAll(int userId, int offset, int categoryId, String orderBy, String search) {
        return fileMapper.listAll(userId, offset, categoryId, orderBy, search);
    }
    ...
    public boolean upload(int categoryId, String tag, String description, String prefix, MultipartFile multipartFile,
        ...
        if (user.getIsUploadable() == 1) {
            ...
            long maxSize = Formatter.sizeToLong(WangpanApplication.settings.getStringUseEval(ConfigConsts
                    .FILE_MAX_SIZE_OF_SETTING));
            long size = multipartFile.getSize();
            boolean fileExists = localUrlExists(localUrl);
            //检测标签是否合法
            if (WangpanApplication.settings.getBooleanUseEval(ConfigConsts.TAG_REQUIRE_OF_SETTING)) {
                String[] tags = Checker.checkNull(tag).split(ValueConsts.SPACE);
                if (tags.length <= WangpanApplication.settings.getIntegerUseEval(ConfigConsts.TAG_SIZE_OF_SETTING)) {
                    int maxLength = WangpanApplication.settings.getIntegerUseEval(ConfigConsts.TAG_LENGTH_OF_SETTING);
                    for (String t : tags) {
            ...
            //是否可以上传
            boolean canUpload = !multipartFile.isEmpty() && size <= maxSize && Pattern.compile(WangpanApplication
                    .settings.getStringUseEval(ConfigConsts.FILE_SUFFIX_MATCH_OF_SETTING)).matcher(suffix).matches()
                    && (Checker.isNotExists(localUrl) || !fileExists || WangpanApplication.settings.getBooleanUseEval
            ...
            if (canUpload) {
                String visitUrl = getRegularVisitUrl(Checker.isNotEmpty(prefix) && user.getPermission() > 1 ? prefix
                        : WangpanApplication.settings.getStringUseEval(ConfigConsts.CUSTOM_LINK_RULE_OF_SETTING), user,
                ...
                try {
                    ...
                    boolean isSuccess = fileMapper.insertFile(file);
                    if (isSuccess) {
                        long fileId = fileMapper.getIdByLocalUrl(localUrl);
                        if (fileId > 0) {
    ...
    public boolean localUrlExists(String localUrl) {
        return fileMapper.checkLocalUrl(localUrl) > 0;
    }

    @Override
    public boolean visitUrlExists(String visitUrl) {
        return fileMapper.checkVisitUrl(visitUrl) > 0;
    }

    @Override
    public long getFileId(String localUrl) {
        try {
            return fileMapper.getIdByLocalUrl(localUrl);
        } catch (Exception e) {
            return 0;
        }
    }
    ...
    public List<FileBasicRecord> listBasicAll(String user, String file, String category, int offset) {
        return (List<FileBasicRecord>) ServiceUtils.invokeFileFilter(fileMapper, "listBasicBy", user, file, category,
                offset);
    }
}
